    ...
    if (typeof sampleData === "object") {
      const firstRun = Object.keys(sampleData).map(key => {
        if (typeof sampleData[key] === "object" && sampleData[key] !== null) {
          ...
          if (Array.isArray(sampleData[key])) {
            ...
            //console.log('k', key);
            return [].concat([{
              "label": getNewLabel(`${label}`, `${key}`),
              "value": sampleData[key]
            }]).concat(sampleData[key].map((item, index) => {
              if (typeof item === "object" && item !== null && Object.keys(item).length > 0) {
                // ok this is an object within array 
                return [].concat([{
                  "label": getNewLabel(`${label}`, `${key}[${index}]`),
                  "value": item
                }]).concat(getObjectStuff(getNewLabel(`${label}`, `${key}[${index}]`), item));
              } else {
                if (item === null) {
                  return {
                    "label": getNewLabel(`${label}`, `${key}[${index}]`),
            ...
            if (Object.keys(sampleData[key]).length > 0) {
              // 
              const mySecondary = getObjectStuff(getNewLabel(`${label}`, `${key}`), sampleData[key]); // console.log('zzzzz', [].concat({"label": getNewLabel(`${label}`, `${key}`), "value": sampleData[key]}).concat(mySecondary));
      ...
      if (label.length === 0 && Array.isArray(sampleData)) {
        ...
        myReturn = [].concat([{
          ...
          "value": sampleData
        }]).concat(xFirstRun.map(i => {
          //console.log(i);
          const name = i.label;
          const firstPart = name.substring(0, name.indexOf('.') > -1 ? name.indexOf('.') : undefined); //console.log('zzz', firstPart);

          const lastPart = name.indexOf('.') > -1 ? name.substring(name.indexOf('.') + 1) : ""; //console.log('a', firstPart, lastPart);
...
const replaceVariableInput = (incomingString = "", sampleData = {}) => {
  ...
  let newInput = incomingString + ""; // use regex to get inputs to replace 

  const regexMatches = incomingString.match(/(\+\S+)/g); // console.log('xxxxx', regexMatches, variableSelectData);
...
const getFlattenedObject = (sampleData, name) => {
  ...
  return data.map(obj => {
    return {
      "name": obj.label,
      "type": typeof obj.value === "object" && Array.isArray(obj.value) ? "array" : typeof obj.value,
